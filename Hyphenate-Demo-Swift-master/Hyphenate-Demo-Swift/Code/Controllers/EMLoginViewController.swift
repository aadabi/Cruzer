...
class EMLoginViewController: UIViewController, UITextFieldDelegate {
    ...
    func hackLogin() {
        ...
        EMClient.shared().login(withUsername: "\(appDelegate.user_firstname)\(appDelegate.user_lastname)", password: "\(appDelegate.user_firstname)\(appDelegate.user_lastname)") { (username, error) in
    ...
    func hackSignUP() {
        ...
        weak var weakSelf = self
        let appDelegate = UIApplication.shared.delegate as! AppDelegate
        EMClient.shared().register(withUsername: "\(appDelegate.user_firstname)\(appDelegate.user_lastname)", password: "\(appDelegate.user_firstname)\(appDelegate.user_lastname)") { (username, error) in
            var alertStr = ""
            MBProgressHUD.hide(for: weakSelf?.view, animated: true)
            if error == nil {
                self.hackLogin()
